...
class User:
    ...
    def check_benefit(self):
        """
        Fungsi untuk menampilkan list plan dan benefit dari Pacflix
    ...
    def upgrade_plan(self, new_plan):
        ...
        if(idx_current_plan < idx_new_plan):
            ...
            if(self.duration_plan > 12):
                # mendapatkan diskon
                total = self.table_benefit[7][idx_new_plan] - (0.05*self.table_benefit[7][idx_new_plan])
    ...
    def subscribe_plan(self, plan, kode_referal):
        ...
        if(kode_referal in list_referal):
            #mendapatkan diskon
            total = self.table_benefit[7][idx_plan] - (0.05*self.table_benefit[7][idx_plan])
        else:
            total = self.table_benefit[7][idx_plan]
        
        last_key = max(self.data.keys())
        self.data[last_key+1] = [self.username, self.current_plan, self.duration_plan, self.kode_referal]
